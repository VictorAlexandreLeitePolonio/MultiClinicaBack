...
public class PatientRepository(AppDbContext db, IUsuarioLogadoService usuario) : IPatientRepository
{
    ...
    public async Task<string?> GetCurrentClinicNameAsync()
        => await db.Clinicas
            .Where(c => c.Id == usuario.ClinicaId)
            .Select(c => string.IsNullOrEmpty(c.NomeFantasia) ? c.Nome : c.NomeFantasia)
            .FirstOrDefaultAsync();

    public async Task<string?> GetCurrentClinicTimeZoneIdAsync()
        => await db.Clinicas
            .Where(c => c.Id == usuario.ClinicaId)
