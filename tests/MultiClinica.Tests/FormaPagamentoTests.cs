...
{
    private static async Task SeedClinicAndUsersAsync(MultiClinicaFactory app)
    {
    ...
    public async Task Create_AdminNomeValido_Retorna201()
    {
        ...
        await SeedClinicAndUsersAsync(app);
        using var client = app.CreateClient();
        await LoginAsync(client, "admin@a.local");

        var response = await client.PostAsJsonAsync("/api/formas-pagamento",
            new CreateFormaPagamentoDto { Nome = "Pix" });

        Assert.Equal(HttpStatusCode.Created, response.StatusCode);
    }

    [Fact]
    public async Task Create_NomeDuplicadoAtivo_Retorna400()
    {
        await using var app = new MultiClinicaFactory();
        await SeedClinicAndUsersAsync(app);
        using var client = app.CreateClient();
        await LoginAsync(client, "admin@a.local");
        await client.PostAsJsonAsync("/api/formas-pagamento", new CreateFormaPagamentoDto { Nome = "Pix" });

        var response = await client.PostAsJsonAsync("/api/formas-pagamento", new CreateFormaPagamentoDto { Nome = "Pix" });
